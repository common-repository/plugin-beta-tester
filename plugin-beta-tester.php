<?php
/*
Plugin Name: Plugin Beta Tester
Description: Lets you easily beta test plugins by notifying you of beta versions. Also lets you upgrade to beta versions from within WordPress.
Version: 0.5
...
class Plugin_Beta_Tester {
	...
	function versions($slug) {
		...
		$api = plugins_api( 'plugin_information', array('slug' => $slug, 'fields' => array('versions' => true)));
		if ( is_object($api) && isset($api->versions) && is_array($api->versions) && count($api->versions) ) {
			...
			$versions_info = (object) array('latest' => $versions[0], 'stable' => $api->version);
